...
<sub>引导式 Web UI：消息气泡视图 · 图片直接显示 · **语音在页面内直接播放** · 文件/链接/引用/位置/通话渲染为卡片 ·
无法离线导出的内容显示为带尺寸与原因的说明卡片（截图为演示数据，真实昵称/路径/内容已替换）</sub>
...
- **图片**：`msg\attach\<会话hash>\<月份>\Img\<md5>*.dat`。旧格式是**整文件单字节 XOR**
  ...
  可完全离线解码；2025-08 之后微信启用了 **V2 容器**（AES-128-ECB，密钥只存在于运行进程内存中），
  ChatTrace 会识别并**如实标注"无法离线导出"**，同时仍显示该图的尺寸与大小。
...
| **媒体** | 图片 dat 解密（旧格式离线直解 / V2 识别标注）、**语音 SILK→WAV 界面内直接播放**、视频与缩略图归档 |
...
| 媒体 | 来源 | 支持情况 |
| --- | --- | --- |
| 图片 | `msg\attach\<会话hash>\<月份>\Img\<md5>*.dat` | ✅ 旧格式单字节 XOR 离线解码（本机实测 key `0xA4`，JPEG/PNG 自动识别），界面内直接显示，带尺寸与大小<br>⚠️ V2 容器（`07 08 56 32 08 07`，2025-08 后新收图片）需运行时密钥，**离线不可解**，界面显示为带尺寸/大小的说明卡片 |
...
tests/                   # 56 个用例：解密往返、跨分片发送者、消息体解析、语音转码、媒体定位
```
...
- **单元测试**：56 个用例覆盖解密往返（用自建 SQLCipher 工厂生成样本）、
  **跨分片发送者解析**、Zstandard 消息体解压与 XML 元数据提取、SILK→WAV 转码往返、
  媒体 dat 解码向量与定位逻辑。
- **真机验证**（本机微信 4.1.12.55，真实账号，48 万条消息）：
...
- **V2 加密图片**（约 2025-08 之后新收图片）：密钥只存在于微信运行进程内存，
  **离线无法解密**。界面会显示该图的尺寸与大小，并注明原因。已在计划中通过扩展
  KeyAgent（运行时提取 + 模板验证）解决。
...
| M5 | V2 图片运行时密钥提取；文件本体关联；分析报告（统计/年度/词频） | 🔭 规划中 |
